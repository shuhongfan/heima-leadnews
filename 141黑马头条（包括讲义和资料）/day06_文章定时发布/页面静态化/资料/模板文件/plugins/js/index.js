...
new Vue({
  ...
  data() {
    return {
      // Minio模板应该写真实接口地址
      baseUrl: 'http://localhost:5001', //'http://172.17.0.140:5001',
      token: '',
      equipmentId: '',
      articleId: '',
      title: '',
      authorId: 0,
      authorApUserId: _authorApUserId,  // 2、从article.ftl模板文件获取_authorApUserId对应的变量
      authorName: '',
      publishTime: '',
      relation: {
        islike: false,
  ...
  methods: {
    ...
    async loadArticleBehavior() {
      const url = `${this.baseUrl}/behavior/api/v1/article/load_article_behavior/`
      const data = { equipmentId: this.equipmentId, articleId: this.articleId, authorId: this.authorId, authorApUserId: this.authorApUserId}
      const config = { headers: { 'token': this.token } }
     ...
     async read() { 
      ...
      // 新增：authorApUserId传递到后端
      const data = {equipmentId: this.equipmentId,articleId:this.articleId,readDuration:0,percentage:0,loadDuration:0}
      const config = { headers: { 'token': this.token } }
      try {
    ...
    async handleClickArticleFollow() {
      ...
      // 新增：authorApUserId传递到后端
      const data = { authorId: this.authorId, authorApUserId: this.authorApUserId, operation: this.relation.isfollow ? 1 : 0, articleId: this.articleId }
      const config = { headers: { 'token': this.token } }
    ...
    async handleClickArticleCollection() {
      const url = `${this.baseUrl}/behavior/api/v1/collection_behavior/`
